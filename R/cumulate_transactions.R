...
#' @param financialCols Character vector corresponding to the name of financial columns in \code{transactions}
#'
#' @export
#' @import data.table
#'
...
#' cumulate_transactions(transactions, financialCols=c("Amount"))

cumulate_transactions <- function(transactions, colCustomerID="CustomerID", colTransactionDate="TransactionDate", financialCols=NULL){
  ...
  transactions.copy <- copy(transactions[, c(colCustomerID, colTransactionDate, financialCols), with=FALSE])
  ...
  if(is.null(financialCols)){
    ...
    expr <- paste0("list(Transactions=.N, ", paste0(financialCols, "=sum(", financialCols, ")", collapse=", "), ")")
  }
  ...
  # Get cumulative sums
  numericCols <- c("Transactions", financialCols)
